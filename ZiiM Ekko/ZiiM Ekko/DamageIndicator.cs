...
namespace ZiiM.Ekko
{
    ...
    "Damage Indicator: Cannot Add '{0}', already added. Please Use Update to change the values.", s);
            ...
            {
                if (!ZiiM.Ekko.Config.Drawing.DamageInd) return;
                ...
                foreach (var aiHeroClient in EntityManager.Heroes.Enemies)
                {
                    ...
                    foreach (var damageItem in _damageItems)
                    {
                        ...
                        //Line.DrawLine(Color.Red, 3, new Vector2(end + (damage > fullbar ? fullbar : damage) + 1, pos.Y - 4), new Vector2(end + (damage > fullbar ? fullbar : damage) + 1, pos.Y + 5));
                        end = pos.X + damage;
                    }
                }
            }
